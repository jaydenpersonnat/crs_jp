...
let invert_pairs : point_pair list -> point_pair list  = 
...
let length (xs : 'a list) : int = 
  List.fold_right (fun _ acc -> succ acc) xs 0 ;; 
...
(* Solution *)
let rec max_list_correct (lst : int list) : int = 
  match lst with 
  | [] -> raise (Invalid_argument "max_list: empty list")
  | [elt] -> elt 
  | hd :: tl -> max hd (max_list tl) ;; 
...
let rec search (key : 'a) (xs : ('a * 'b) list) : 'b option = 
  match xs with 
  | [] -> None 
  | (k, v) :: tl -> if k = key then Some v else search key tl ;; 